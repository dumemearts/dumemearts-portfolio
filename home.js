...
// H2 TEXT STAGGER DESKTOP
  ...
  const headings = document.querySelectorAll('.heading-style-h2.is--smaller');

  headings.forEach((heading) => {
    const chars = heading.textContent.trim().split('');

    // Clear the heading and wrap each character in a span
    heading.innerHTML = chars.map(char => {
      const safeChar = char === ' ' ? '&nbsp;' : char;
      return `<span class="char">${safeChar}</span>`;
    }).join('');

    const charsSpans = heading.querySelectorAll('.char');

    // Create animation
    gsap.fromTo(charsSpans,
      { y: 40, opacity: 0 },
      {
        y: 0,
        opacity: 1,
        ...
        scrollTrigger: {
          trigger: heading,
          start: "top 80%",
          toggleActions: "play none none reset", // play every time
        }
      }
    );
  });
